...
- 设置侧栏精简为通用、Agent 集成、数据同步与备份、高级四类。
...
本版本不包含 Windows 端安装包；Windows 客户端仍处于 Preview。

## 数据与隐私

Release 资产不包含密码、WebDAV 凭据、用户历史、数据库、设置文件、私钥或本机配置。应用数据继续保存在独立的 Application Support 目录中，更新应用不会删除这些数据。
