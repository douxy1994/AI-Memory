...
| 工作台与项目归组 | 已实现 | 已实现核心 |
| 对话搜索与详情 | 已实现 | 已实现核心：来源/项目筛选、按项目/时间线排列、更新/创建/标题排序与可操作详情 |
...
| MCP helper | 已实现：22 项工具 | Preview：22 项工具契约与 win-x64/win-arm64 发布产物已通过；当前提交的打包运行冒烟待 Windows Runner 重跑 |
...
| 签名公开安装包 | 已发布：v0.1.2 universal DMG、本地固定签名与 SHA-256；尚未 Apple Developer ID 公证 | 暂未发布 |
