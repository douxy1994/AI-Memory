...
actor NativeAgentIntegrationStore {
    ...
    /// at either the helper executable or its `Contents/Helpers` directory in
    /// the latter case, so appending another `Contents/Helpers` segment would
    /// produce a path that can never exist.
    static func bundledHelperURL(bundleURL: URL) -> URL {
        if bundleURL.lastPathComponent == "aimemory-mcp" {
            return bundleURL
        }
        if bundleURL.lastPathComponent == "Helpers",
           bundleURL.deletingLastPathComponent().lastPathComponent
               == "Contents" {
            return bundleURL.appendingPathComponent("aimemory-mcp")
        }
